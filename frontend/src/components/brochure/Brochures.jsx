...
const BrochureManager = () => {
  ...
  const modularKitchenCount = brochures.filter(
    (b) => b.category === "modular_kitchen"
  ).length;
  const inPlaceFurnitureCount = brochures.filter(
    (b) => b.category === "inPlace_Furniture"
  ).length;

  const filteredBrochures = brochures.filter((brochure) => {
    const matchesSearch =
      brochure.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
      brochure.keywords.some((keyword) =>
        keyword.toLowerCase().includes(searchTerm.toLowerCase())
      );
    const matchesCategory =
      filterCategory === "all" || brochure.category === filterCategory;
    ...
    <div className="max-w-7xl mx-auto p-6 bg-white">
      <div className="mb-8">
        ...
        <p className="text-gray-600">
          Upload, manage, and organize your brochures
        </p>
      </div>
