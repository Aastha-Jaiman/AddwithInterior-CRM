...
import {
  loginAdmin,
  resetEmailToken as resetAdminEmailToken,
...
export default function LoginPage() {
  ...
  const handleForgotPassword = async (e) => {
    ...
    try {
      ...
      if (
        ...
        // Admin/Staff
        response = await resetAdminEmailToken(forgotEmail);
      } else {
        // Client
        response = await resetClientEmailToken({ email: forgotEmail });
      }
      setForgotSuccess(
  ...
  // Toggle and reset forgot fields when toggling
  const toggleForgot = () => {
        ...
        <div className="w-full max-w-sm sm:max-w-md lg:max-w-lg xl:max-w-xl">
          ...
          <div className="bg-white/80 backdrop-blur-xl shadow-xl rounded-xl sm:rounded-2xl p-6 sm:p-8 lg:p-10 border border-white/20">
            ...
            >
              ...
              {!showForgot && error && (
                <div className="bg-red-50 border border-red-200 text-red-700 px-3 sm:px-4 py-2 sm:py-3 rounded-xl text-xs sm:text-sm flex items-start sm:items-center gap-2">
                  <svg
                    className="w-4 h-4 text-red-500 flex-shrink-0 mt-0.5 sm:mt-0"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
            ...
            {showForgot && (
              <form
                ...
                className="space-y-3 mt-4 mb-2"
                autoComplete="off"
              >
                <div>
                  <label className="block text-xs sm:text-sm mb-2 text-gray-700">
                ...
                <button
                  ...
                  className="w-full bg-gradient-to-r from-gray-400 to-blue-500 hover:from-blue-600 hover:to-blue-700 text-white font-semibold py-2 px-4 text-sm rounded-xl disabled:opacity-50"
                >
                ...
                <button
                  type="button"
                  onClick={toggleForgot}
                  className="text-xs sm:text-sm text-gray-600 mt-2 hover:underline"
                  tabIndex={-1}
                >
                  Back to Login
                </button>
          ...
          <div className="mt-6 sm:mt-8 text-center px-4">
            <p className="text-xs sm:text-sm text-gray-600">
              ...
              <a
                href="#"
                className="font-medium text-blue-600 hover:text-blue-500 transition-colors block sm:inline mt-1 sm:mt-0"
              >
                Contact support
              </a>
            </p>
