...
// import { getAllClientsEmail, getProjectsByClientEmail } from '@/services/quotation.services';

// export default function AddPaymentPage({ editMode = false, existingPayment = null, onSubmit }) {
...
//   // Fetch projects when clientId (actually client email) changes
//   useEffect(() => {
//     if (!clientId) {
//       setProjects([]);
...
//         // fetch projects for that client email
...
//   //   // validation: amount should not exceed finalBudget
...
//           <p className="text-gray-600">Record new payment transactions for your projects</p>
//         </div>
...
//                         <span className="text-gray-900">{clientDetails.phone || 'Not provided'}</span>
//                       </div>
//                       <div className="flex items-start">
//                         <span className="font-medium text-gray-600 w-20">Aadhar:</span>
//                         <span className="text-gray-900">{clientDetails.aadharCardNumber || 'Not provided'}</span>
//                       </div>
//                     </div>
...
//                         <span className="text-gray-900">{projectDetails.title}</span>
//                       </div>
//                       <div className="flex items-start">
//                         <span className="font-medium text-gray-600 w-24">Category:</span>
//                         <span className="px-2 py-1 bg-blue-100 text-blue-800 text-sm rounded-full">{projectDetails.category}</span>
//                       </div>
//                       <div className="flex items-start">
//                         <span className="font-medium text-gray-600 w-24">Status:</span>
//                         <span className="px-2 py-1 bg-green-100 text-green-800 text-sm rounded-full">{projectDetails.status}</span>
//                       </div>
//                     </div>
//                     <div className="space-y-3">
//                       <div className="flex items-start">
//                         <span className="font-medium text-gray-600 w-24">Location:</span>
//                         <span className="text-gray-900">{projectDetails.location}</span>
//                       </div>
//                       <div className="flex items-start">
//                         <span className="font-medium text-gray-600 w-24">Budget:</span>
//                         <span className="text-gray-900 font-semibold text-lg">₹{projectDetails.finalBudget?.toLocaleString() || 'Not set'}</span>
//                       </div>
//                       <div className="flex items-start">
//                         <span className="font-medium text-gray-600 w-24">Client:</span>
//                         <span className="text-gray-900">{projectDetails.client?.name} ({projectDetails.client?.email})</span>
//                       </div>
//                     </div>
//                   </div>
//                 </div>
...
//                       Maximum allowed: ₹{projectDetails.finalBudget.toLocaleString()}
//                     </p>
//                   )}
//                 </div>
...
export default function AddPaymentPage({ editMode = false, existingPayment = null, onSubmit }) {
  ...
  useEffect(() => {
    if (editMode && existingPayment) {
      ...
      setMessage(existingPayment.message || '');
      if (existingPayment.client?.email) {
        setClientId(existingPayment.client.email);
      }
      if (existingPayment.project?._id) {
        setProjectId(existingPayment.project._id);
      }
    }
  }, [editMode, existingPayment]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!editMode && Number(amount) > Number(projectDetails?.finalBudget || 0)) {
      setErr('Amount cannot be greater than final budget');
      setLoading(false);
      return;
    }
    ...
    try {
      if (editMode) {
        ...
        await onSubmit(existingPayment._id, {
          amount: Number(amount),
          message,
          clientId: clientDetails?._id,
          projectId,
